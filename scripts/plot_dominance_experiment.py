...
for epoch in range(0, 181, 20):
    rc = subprocess.call(f"./main.py --dominance --dominance_save_name {args.dominance_save_name} --dominance_file_name independent \
                     --arch {args.arch} --view-size {args.view_size} --resume /home/ubuntu/AWSCVCriticalPeriodStereo/critical_stereo/models/{main_log}_{epoch}.pth",
                         shell=True)
...
for log in exlog['runs']:
    rc = subprocess.call(f"./main.py --dominance --dominance_save_name {args.dominance_save_name} --dominance_file_name resume \
                     --arch {args.arch} --view-size {args.view_size} --resume /home/ubuntu/AWSCVCriticalPeriodStereo/critical_stereo/models/{log}.pth", shell=True)

rc = subprocess.call(f"python plot_experiment.py {args.experiment} --save_dir {args.dominance_save_name} -f pdf", shell=True)